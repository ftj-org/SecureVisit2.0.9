...
class log_search1_M{
	...
	*/
	function __construct(){		
		try{
			...
			$this->logSOneView();
			ToLog::ToLogs(DEBUG,__FILE__,__LINE__,"実例化");
		}
		catch( Exception $e ){
	...
	* 
	*/
	function logSOneView(){		
		try{
			/*try{
				$strSql  = " DROP VIEW logsoneview";
			...
			//ビューを作る.
			$strSql  = " CREATE VIEW logsoneview AS ";
	...
	*/
	function readTableList($orderBy,$condition,$offset,$pageNum){
		try{
			// ページビューのインスタンス化.
			$pageObj = new PageView("(SELECT  DISTINCT ON(userid) * FROM logsoneview ) as temp ",$pageNum,$offset ,$this->objDb);
	...
	* @return		void.
	*/
	function exportLogSOneCSV($orderBy,$condition){
		try{
			$arrNewTitle = $this->readTableNew();
			$strTitleSql = "";
			...
			$strSql .= " memo ";
			$strSql .= $strTitleSql;
			$strSql .= " FROM (SELECT  DISTINCT ON(userid) * FROM (".$this->viewCreateSql.") as logsoneview ) as temp ";
			$strSql .= $condition.$orderBy ;
			//$res = $this->objDb->ExecuteArrayCommand($strSql);
			
			$fileName = G_DOC_TMP.SESSION::Get("SESS_USERID").SESSION::Get("SESS_LOGINDATE")."LogSOneCSV.csv";
